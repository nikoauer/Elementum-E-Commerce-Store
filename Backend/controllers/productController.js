...
const addProduct = asyncHandler(async (req, res) => {
  try {
    const { brand, name, description, price, category, quantity, image, CountInStock } =
      req.body;
...
const filterProducts = asyncHandler(async (req, res) => {
  try {
    const {checkd, radio} = req.body
    let args = {}
    if(checke.length > 0) args.category = checked;
...
export { addProduct, filterProducts, getNewProducts, updateProductDetails, deleteProduct, getAllProducts, getTopProducts, fetchProductById, fetchallProducts, addReview };
