...
const Order = () => {
  ...
  return isLoading ? (
    ...
    <div className="px-4 sm:px-6 lg:px-8 w-full max-w-screen-xl">
      ...
      <div className="fixed left-0 top-0 hidden w-1/2 bg-white lg:block" aria-hidden="true" />
      <div className="fixed right-0 top-16 hidden h-full w-1/2 bg-sky-700 lg:block" aria-hidden="true" />
      ...
      <main className="relative mx-auto grid max-w-7xl grid-cols-1 gap-x-16 lg:grid-cols-2 lg:px-8">
            ...
            <ul role="list" className="divide-y divide-white divide-opacity-10 text-sm font-medium">
              {order.orderItems.map((item, index) => (
                <li key={index} className="flex items-start space-x-4 py-6">
                  ...
                  <p className="flex-none text-base font-medium text-white">$ {(item.qty * item.price).toFixed(2)}</p>
                </li>
              ))}
            </ul>
        ...
        >
            <div className="mx-auto max-w-2xl px-4 lg:max-w-none lg:px-0">
              <div>
                <h3 id="contact-info-heading" className="text-xl font-semibold text-gray-900">
                  Order ID: {order._id}
                </h3>
                ...
                <div>
                  <p className="block text-md font-lg text-gray-800">
                    <strong>Name:</strong><span> {order.user.username}</span>
                  </p>
                  </div>
                  <div className="mt-2">
                  <p className="block text-md font-lg text-gray-800">
                    <strong>Email address:</strong><span> {order.user.email}</span>
                  </p>
                  </div>
                  <div className="mt-2">
                  <p className="block text-md font-lg text-gray-800">
                    <strong>Address:</strong><span> {order.shippingAddress.address}, {order.shippingAddress.city} {order.shippingAddress.postalCode}, {order.shippingAddress.state}, {order.shippingAddress.country}</span>
                  </p>
                  </div>
                </div>
              </div>
